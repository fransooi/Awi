...
class ConnectorLangageAoz extends awiconnector.Connector
{
	...
	async connect( options )
	{
		...
		do
		{
			var answer = await this.awi.utilities.loadJSON( this.awi.config.getEnginePath() + '/data/language.json', { encoding: 'utf8' } );
			...
			if ( this.aozPath )
			{
				var aozConfigPath = this.awi.utilities.normalize( this.aozPath + '/AOZ_Studio/.aozstudio/aoz.hjson' );
				var answer = await this.awi.utilities.loadHJSON( aozConfigPath );
				this.aozConfig = answer.data;
				if ( !this.aozConfig )
					break;
				var userConfigPath = this.awi.utilities.normalize( this.aozPath + '/AOZ_Studio/.aozstudio/user.hjson' );
				answer = await this.awi.utilities.loadHJSON( userConfigPath );
				this.userConfig = answer.data;
				if ( !this.userConfig )
					break;
			}
			this.connected = true;
	...
	async getImportPaths()
	{
		...
		if ( this.aozConfig )
		{
			toScan.push( this.awi.utilities.normalize( this.aozConfig.runPaths[ 'drive' ] ) );
			toScan.push( this.awi.utilities.normalize( this.aozConfig.runPaths[ 'applications' ] ) );
			toScan.push( this.awi.utilities.normalize( this.aozConfig.runPaths[ 'tutorials' ] ) );
			toScan.push( this.awi.utilities.normalize( this.aozConfig.runPaths[ 'games' ] ) );
			toScan.push( this.awi.utilities.normalize( this.aozConfig.runPaths[ 'demos' ] ) );
		}
		var self = this;
		async function checkPath( path, type )
		{
			var checkList =
			{
				image: [ '/images', '/Images', '/1.Images', '1.images' ],
				sound: [ '/sounds', '/Sounds', '/samples', '/Samples','/5.Sounds', '/5.Sounds', '/5.Samples', '/5.Samples' ],
				music: [ '/musics', '/Musics', '/3.musics', '/3.Musics' ],
				video: [ '/assets', '/Assets' ],
				json: [ '/assets', '/Assets' ],
				text: [ '/assets', '/Assets' ],
				asset: [ '/assets', '/Assets' ],
			}
			var list = checkList[ type ];
			if ( list )
			{
				for ( var l = 0; l < list.length; l++ )
				{
					var destPath = self.awi.utilities.normalize( path + list[ l ] );
					var answer = await self.awi.system.exists( destPath );
					if ( answer.success )
					{
						return destPath;
					}
		...
		if ( this.userConfig )
		{
			if ( this.userConfig.currentFile )
			{
				var path = self.awi.utilities.parse( this.userConfig.currentFile ).dir + '/resources';
				importPaths[ 'image' ] = await checkPath( path, 'image' );
				importPaths[ 'sound' ] = await checkPath( path, 'sound' );
				importPaths[ 'video' ] = await checkPath( path, 'video' );
				importPaths[ 'music' ] = await checkPath( path, 'music' );
				importPaths[ 'json' ] = await checkPath( path, 'json' );
				importPaths[ 'text' ] = await checkPath( path, 'text' );
				importPaths[ 'asset' ] = await checkPath( path, 'asset' );
			}
		}
		return { success: true, data: { toScan: toScan, importPaths: importPaths } };
	}
	async import( path, options = {} )
	{
		...
		var answer = await this.getImportPaths();
		var importPaths = answer.data;
		var destinationPath = importPaths.importPaths[ type ];
		answer = await this.awi.system.exists( destinationPath );
		if ( destinationPath && answer.success )
		{
